...
export async function replyAndResolveAddressedThreads(gh: GhExecutor, config: GithubPrGoalConfig, event: CompleteEvent): Promise<string[]> {
  ...
  for (const threadId of threadIds) {
    if (!isResolvableThreadId(threadId)) continue;
    attempted++;
  ...
  if (attempted > 0 && failures > 0 && resolved.length === 0) throw new Error("Failed to reply/resolve addressed GitHub review threads");
  if (resolved.length) await gh.run(["pr", "view", String(config.prNumber), "--repo", repo, "--json", "number"]);
  return resolved;
}

function isResolvableThreadId(value: string): boolean {
